...
$club = isset($data['userData']) ? $data['userData']->club : ''; //72
...
<?php 
	...
	$lactGultenOptions = [
		''=>'',
		'Yes – I\'m gluten intolerant' => "Yes – I'm gluten intolerant",
		'Yes – I\'m lactose intolerant' => "Yes – I'm lactose intolerant",
		'Yes, I\'m both gluten & lactose intolerant' => "Yes, I'm both gluten & lactose intolerant",
	];
	$specOccasOptions = [
		''=>'',
		'Birthday' => 'Birthday',
	...
	$yesNoEmptyOptions = [''=>'','No'=>'No', 'Yes'=>'Yes'];
	$seatingPreference = [
	...
	if($club == 0)
		$deptOpt = 'On group date - Sunday, June 10';
	else 
		$deptOpt = 'On President\'s Club departure date - Friday, June 8';
	...
	$inputFields = [
		'Winner' => [
			...
			'Do you have a valid US Visa, OR Do you hold a valid US Resident Card/Green Card?' => ['name'=>'us_visa','type'=>'dropdown','options'=>'yesNoEmptyOptions','class'=>'form-control','value'=>$us_visa],
			'Are you able to obtain a US Visa?' => ['name'=>'obtain_visa','type'=>'dropdown','options'=>'yesNoEmptyOptions','class'=>'form-control','value'=>$obtain_visa],
		...
		'Companion' => [
			...
			'Do you have a valid US Visa, OR Do you hold a valid US Resident Card/Green Card?' => ['name'=>'us_visa_guest','type'=>'dropdown','options'=>'yesNoEmptyOptions','class'=>'form-control','value'=>$us_visa_g],
			'Are you able to obtain a US Visa?' => ['name'=>'obtain_visa_guest','type'=>'dropdown','options'=>'yesNoEmptyOptions','class'=>'form-control','value'=>$obtain_visa_g],
	...
	// remove shirt size field if user not persident club
	if($club == 0){
		unset($inputFields['Winner']['Shirt Size']);
		unset($inputFields['Companion']['Shirt Size']);
	}
	/*echo '<pre>';
	print_r($inputFields);
	exit;*/
?>
    ...
    <script type="text/javascript">
    	...
    	if(!isset($data['userData']->noguest) || $data['userData']->noguest != 'Yes') {?>
			$('.reg-quick-view').find('.Companion').find('input, textarea, select').prop('disabled', true).val("").find('option[selected="selected"]').html("");
		<?php } ?>
    	...
    	$(document).on('change','select[name="us_visa"], select[name="us_visa_guest"]', function(e){
    		var fieldsHidOnUsVisa = ['obtain_visa'];
    		hideFields(fieldsHidOnUsVisa, this, 'No');
    	})
